...
- HTML 리포트를 report_latest.html 로 저장 → GitHub Pages 배포
- 카카오톡으로 요약 + 리포트 링크 전송
"""
...
from recommend import scan, STOCKS, ETFS, get_sector_performance, ask_gemini, save_html, scan_industry
...
# report_latest.html 로 복사 (GitHub Pages 배포용)
shutil.copy(html_path, "report_latest.html")
print("✅ report_latest.html 생성")
...
for i, s in enumerate(top5, 1):
    chg = s.get("change", 0)
    msg += f"{i}위 {s['name']} {s['price']:,}원 ({chg:+.1f}%)\n"
msg += f"\n📊 리포트 보기 → 아래 버튼 클릭"
